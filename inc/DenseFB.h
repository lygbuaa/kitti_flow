...
class DenseFB : public KittiFlowBase
{
private:
    uint64_t average_us_ = 0;

public:
    DenseFB(const std::string& img_path, const std::string& gt_path)
    : KittiFlowBase(img_path, gt_path)
    {
        LOG(INFO) << "DenseFB init.";
    }
    ...
    std::string gen_output_path(const std::string input_img_path){
        ...
        std::string output_path = FLAGS_output_img_path + "/" + file_name + ".dense_fb.png";
        return output_path;
    }
    ...
    std::vector<float> run_once(unsigned int counter, std::map<std::string, std::string>& img_pair, bool enable_visual=false){
        ...
        */
        ...
        cv::calcOpticalFlowFarneback(prev_gray, this_gray, flow, 0.5, 3, 21, 3, 5, 1.1, 0);
