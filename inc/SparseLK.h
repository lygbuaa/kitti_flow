...
public:
    ...
    void run_all(bool enable_visual=false){
        ...
        while(true){
            ...
            run_once(counter, img_pair, enable_visual);
            ++ counter;
        }
    }

    void run_once(unsigned int counter, std::map<std::string, std::string>& img_pair, bool enable_visual=false){
        ...
        if(enable_visual){
            ...
            for(uint i = 0; i < p0.size(); i++){
                ...
                if(status[i] == 1) {
                    ...
                    cv:: circle(this_img, p1[i], 5, colors[i], -1);
                }
            }
            LOG(INFO) << "[" << counter << "] tracked features: " << good_new.size();

            cv::Mat vis_img;
            cv::add(this_img, mask, vis_img);
